...
def move_bullets(yellow_bullets, red_bullets, yellow, red):
    for bullet in yellow_bullets:
        ...
        if red.colliderect(bullet):
            ...
            HIT_SOUND = MIXER.music.load("Assets\Assets_Grenade+1.mp3")
            MIXER.music.play()
        if bullet.x > W:
            yellow_bullets.remove(bullet)
    for bullet in red_bullets:
    ...
    while run:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_LCTRL and len(yellow_bullets) < SHOTS_ALLOWED and not game_over:
                    ...
                    MIXER.music.play()
                if event.key == pygame.K_RCTRL and len(red_bullets) < SHOTS_ALLOWED and not game_over:  
